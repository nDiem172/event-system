...
export default function VerifyEmailPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      setErrorMessage('Thiếu token xác thực trong đường dẫn.');
      setStatus('error');
      return;
    }
    authAPI
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '70vh' }}>
      <div style={{ textAlign: 'center', background: '#fff', padding: 48, borderRadius: 14, boxShadow: '0 4px 20px rgba(0,0,0,0.1)', maxWidth: 420 }}>
        ...
        {status === 'error' && <><div style={{ fontSize: 60 }}>❌</div><h2 style={{ color: '#e74c3c' }}>Không xác thực được</h2><p style={{ color: '#666' }}>{errorMessage || 'Link đã hết hạn hoặc đã được sử dụng.'}</p><Link to="/login" style={{ color: '#2E75B6' }}>Quay về đăng nhập</Link></>}
      </div>
    </div>
  );
}
